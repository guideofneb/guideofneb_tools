...
const Step8 = (input_data: LatexAndQuestionData): LatexAndQuestionData => {
    let questionData = input_data.questionData;
    if (questionData[1][2] !== 0) {
        let rowoperationdata = makeZero(ROW.R2, ROW.R3, questionData, questionData[1][2]);
