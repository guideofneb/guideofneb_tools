...
math_function.lex = function(inp, tokens) {
    ...
    for(i = 0; i < nodes.length; i++){
        var node = nodes[i];
        if (node.type === 14) {
            if (i > 0 && i < nodes.length - 1 && nodes[i + 1].type === 1 && (nodes[i - 1].type === 1 || nodes[i - 1].type === 6)) throw new math_function.Exception("Unexpected Space");
            continue;
        }
...
export { formula_evaluator as default };
